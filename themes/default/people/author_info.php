...
<section id="author_info" class="padded">
	...
	<section class="attributed_content">
		...
		<?php  if ($user->POD->isAuthenticated() && $user->POD->currentUser()->get('id') != $user->get('id')) { ?>
			<a href="#toggleFlag" data-flag="friends" data-person="<?= $user->id; ?>" data-active="Stop Following" data-inactive="Follow" class="person_output_follow_button <?php  if ($user->hasFlag('friends',$POD->currentUser())){?>active<?php  } ?>">Follow</a>
		<?php  } ?>
	</section>
