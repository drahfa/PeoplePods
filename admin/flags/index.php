		...
		<div class="list_panel">
			...
			<ul id="content_type">
				...
				<?php  if ($oflags) foreach ($oflags as $flag=>$type) { ?>
					<li <?php  if ($_GET['flag']==$flag && $_GET['direction']=="out") {?>class="active"<?php  } ?>><a href="?userId=<?= $_GET['userId']; ?>&flag=<?= $flag; ?>&type=<?= $type ?>&direction=out"><?= $flag; ?> &rarr;</a></li>
				<?php  } ?>
				<?php  if ($iflags) foreach ($iflags as $flag=>$type) { ?>
					<li <?php  if ($_GET['flag']==$flag && $_GET['direction']=="in") {?>class="active"<?php  } ?>><a href="?userId=<?= $_GET['userId']; ?><?php  if ($_GET['contentId']){?>&contentId=<?= $_GET['contentId']; ?><?php  } ?>&flag=<?= $flag; ?>&type=user&direction=in"><?= $flag; ?> &larr;</a></li>
				<?php  } ?>
			</ul>
			<?php  } ?>
	<?php 	
		if (isset($results)) { ?>
			<h1 class="column_padding"><a href="<?php  $POD->podRoot(); ?>/admin/flags/">Flags</a>: <a href="index.php?<?php   if(isset($content)) { echo 'contentId='.$content->id; } else if (isset($user)) { echo 'userId=' . $user->id; }?>"><?php  if (isset($content)) { echo $content->headline; } else if (isset($user)) { echo $user->nick; } ?></a> &#187; <?= $description; ?></h1>
		...
		<?php  } else { 
			...
			if ($oflags || $iflags) { ?>
				<div class="empty_list">
					...
					<h1><a href="<?php  $POD->podRoot(); ?>/admin/flags/">Flags</a>: <?= $content ? $content->headline : $user->nick; ?></h1>
					<?php  if ($oflags) { ?>
						<p>This person has added flags to things:</p>
						<ul class="flag_list">
						<?php  foreach($oflags as $flag=>$type) { ?>
							<li><a href="?userId=<?= $_GET['userId']; ?>&flag=<?= $flag; ?>&type=<?= $type ?>&direction=out"><strong><?= $flag; ?></strong></a> <?php  if ($type=="content") { echo $POD->pluralize($user->flaggedCount($flag),"@number piece of content","@number pieces of content"); } else { echo $POD->pluralize($user->flaggedCount($flag),"@number person","@number people"); } ?></li>	
						<?php  } ?>
						</ul>
					<?php  }
					...
					<?php  } else { ?>
						...
						<?php 
							...
							foreach ($flags as $flag) { ?>
								<li><a href="<?php  $POD->podRoot(); ?>/admin/flags/?flag=<?= $flag['name']; ?>&type=<?= $flag['type']; ?>"><?php  if ($flag['type']=="content") { echo $POD->pluralize($flag['count'],'@number piece of content','@number pieces of content'); } else { echo $POD->pluralize($flag['count'],'@number person','@number people'); } ?> flagged <?= $flag['name']; ?></a></li>
							<?php  } ?>					
						<?php  } ?>
						</ul>
