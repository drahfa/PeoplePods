...
*/
	...
	<div id="options">
		<div class="stats option_set">
			<div class="stat">
				<a href="<?php  $POD->podRoot(); ?>/admin/people/search.php?mode=newest"><?= $POD->pluralize($stats['members_total'],'<span class="number">@number</span> member','<span class="number">@number</span> members'); ?></a>
				<?= $stats['members_today']; ?> joined today<br /> 
				<?= $stats['members_week']; ?> over the last week
			</div>
			<div class="stat">
				<a href="<?php  $POD->podRoot(); ?>/admin/people/search.php?mode=last" title="Members active during the last 30 days"><?= $POD->pluralize($stats['visits_total'],'<span class="number">@number active</span>','<span class="number">@number active</span>'); ?></a>
				<?= $stats['visits_today']; ?> visited today<br />
				<?= $stats['visits_week']; ?> over the last week
			</div>
			<div class="stat">
				<a href="<?php  $POD->podRoot(); ?>/admin/content/search.php"><?= $POD->pluralize($stats['content_total'],'<span class="number">@number</span> post','<span class="number">@number</span> posts'); ?></a>
				<?= $stats['content_today']; ?> created today<br />
				<?= $stats['content_week']; ?>  over the last week
			</div>	
			<div class="stat">
				<a href="<?php  $POD->podRoot(); ?>/admin/comments/"><?= $POD->pluralize($stats['comments_total'],'<span class="number">@number</span> comment','<span class="number">@number</span> comments'); ?></a>
				<?= $stats['comments_today']; ?> posted today<br />
				<?= $stats['comments_week']; ?>  over the last week
			</div>	
			<div class="stat">
				<a href="<?php  $POD->podRoot(); ?>/admin/files/"><?= $POD->pluralize($stats['files_total'],'<span class="number">@number</span> file','<span class="number">@number</span> files'); ?></a>
				<?= $stats['files_today']; ?> uploaded today<br />
				<?= $stats['files_week']; ?> over the last week
			</div>	
			<div class="stat">
				<a href="<?php  $POD->podRoot(); ?>/admin/groups/search.php"><?= $POD->pluralize($stats['groups_total'],'<span class="number">@number</span> group','<span class="number">@number</span> groups'); ?></a>
