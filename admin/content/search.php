<?php
	...
	<div class="list_panel">
		...
		<?php  if (@$_GET['userId']) { 
			...
			?><h1>Content created by <?= $person->nick; ?></h1><?php 
		} ?>

		<div id="searchoptions" style="display: none;">
			<form method="get" action="<?php  $POD->podRoot(); ?>/admin/content/search.php">
		...
		<ul id="content_type">
			<li>Content Type:</li>
			<li <?php  if (!isset($_GET['type'])) { ?>class="active"<?php  } ?>><A href="search.php?<?= $tparameters; ?>">All</a></li>
			<?php  foreach ($types as $type) { ?>
				<li <?php  if (isset($_GET['type']) && $_GET['type']==$type) {?>class="active"<?php  } ?>><a href="?type=<?= $type; ?><?= $tparameters; ?>"><?= $type; ?></a></li>
			<?php  } ?>
		</ul>
		...
		<?php  $docs->output('short','content_header','table_pager',null,'No content found',$parameters); ?>
